...
df_study1 %>%
  # filter(ola==1) %>%
  select(marchar, # distributive justice
        starts_with("barrio_") 
         ) %>% 
  psych::mixedCor() 
...
# colnames(cor_full) <-c("(A)", "(B)","(C)","(D)")
#Plot the matrix using corrplot
png("output/graphs/corr2.png",width=600,height=300, pointsize = 10)
corrplot::corrplot(cor_full,
  method = "color",
  addCoef.col = "#000390",
  type = "upper",
...
h8 <- "ola+edad+rm+cohesion_barrial+sociabilidad_barrial+satisfaccion_residencial"
h9 <- "ola+edad+rm+cohesion_barrial+sociabilidad_barrial+satisfaccion_residencial+educ+quintil1+sexo+ess+pos_id"
...
texreg::htmlreg(list(marchar1, marchar5, marchar6, marchar7, marchar8, marchar9),
                custom.note = "*** p < 0.001; ** p < 0.01; * p < 0.05", 
